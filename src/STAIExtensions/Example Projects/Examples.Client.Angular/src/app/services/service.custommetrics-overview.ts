...
import { BaseCustomMetricsOverviewService, CustomMetricsOverviewView } from '../data/view.custommetrics-overview';
import { STAIExtensionsService } from './staiextensions-data-service';

@Injectable({
  providedIn: 'root'
})
export class CustomMetricsOverviewService implements BaseCustomMetricsOverviewService {
  public View$ = new BehaviorSubject<CustomMetricsOverviewView>(null!);
  public ViewId$ = new BehaviorSubject<string>(null!);
  public SelectedCloudFilters: string[]  = [ '-1' ];

  private ViewTypeName: string = "CustomMetricsView";
    ...
    if (this.ViewId$.value === null) {
      this.stAIService.CreateView$(this.ViewTypeName).then((view) => {
        ...
        this.stAIService.GetView$(view.id).then((getViewResponse) => {
          this.View$.next(getViewResponse  as CustomMetricsOverviewView);
